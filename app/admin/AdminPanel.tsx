...
type Tab = "inicio" | "membresia" | "suscripciones" | "ebooks" | "recursos";
...
export function AdminPanel({ initialContent, initialSubscriptions }: { initialContent: ContentBundle; initialSubscriptions: AdminSubscriptionRecord[] }) {
  ...
  async function uploadEbook(ebook: EbookRecord, file: File | undefined) {
    if (!file) return;
    if (!ebook.id) { setNotice("Primero guardá el ebook y después subí el PDF."); return; }
    setSaving(true); setNotice("");
    try {
  ...
  return (
    <div className="admin-workspace">
      ...
      <section className="admin-content">
        <div className="admin-heading"><div><p className="eyebrow">Backoffice</p><h1>{tab === "inicio" ? "Inicio y contacto" : tab === "membresia" ? "Membresía" : tab === "suscripciones" ? "Suscripciones" : tab === "ebooks" ? "Ebooks" : "Recursos de socias"}</h1><p>{tab === "recursos" ? "Publicá clases, actividades, audios y guías dentro del área privada." : tab === "suscripciones" ? "Consultá quién tiene acceso, cuándo renueva y qué alumnas solicitaron la baja." : "Editá el contenido con lenguaje simple. Guardá cuando esté listo."}</p></div>{(tab === "inicio" || tab === "membresia") && <button className="button button--dark" disabled={saving} onClick={() => persist({ type: "settings", values: content.settings })}>{saving ? "Guardando…" : "Guardar cambios"}</button>}{tab === "ebooks" && <button className="button button--outline" onClick={addEbook}>+ Agregar ebook</button>}{tab === "recursos" && <button className="button button--outline" onClick={addResource}>+ Agregar recurso</button>}</div>
